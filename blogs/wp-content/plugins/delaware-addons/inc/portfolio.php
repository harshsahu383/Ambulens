...
class Delaware_Portfolio {
	...
	public function register_post_type() {
		...
		$portfolio_permalink = empty( $permalinks['portfolio_base'] ) ? _x( 'portfolio', 'slug', 'delaware' ) : $permalinks['portfolio_base'];
		$portfolio_page_id   = get_option( $this->option . '_page_id' );
		$portfolio_category_base = empty( $permalinks['portfolio_category_base'] ) ? _x( 'portfolio-category', 'slug', 'delaware' ) : $permalinks['portfolio_category_base'];

		register_post_type( $this->post_type, array(
			'description'         => esc_html__( 'Portfolio Items', 'delaware' ),
			'labels'              => array(
				'name'                  => esc_html__( 'Portfolio', 'delaware' ),
				'singular_name'         => esc_html__( 'Project', 'delaware' ),
				'menu_name'             => esc_html__( 'Portfolio', 'delaware' ),
				'all_items'             => esc_html__( 'All Projects', 'delaware' ),
				'add_new'               => esc_html__( 'Add New', 'delaware' ),
				'add_new_item'          => esc_html__( 'Add New Project', 'delaware' ),
				'edit_item'             => esc_html__( 'Edit Project', 'delaware' ),
				'new_item'              => esc_html__( 'New Project', 'delaware' ),
				'view_item'             => esc_html__( 'View Project', 'delaware' ),
				'search_items'          => esc_html__( 'Search Projects', 'delaware' ),
				'not_found'             => esc_html__( 'No Projects found', 'delaware' ),
				'not_found_in_trash'    => esc_html__( 'No Projects found in Trash', 'delaware' ),
				'filter_items_list'     => esc_html__( 'Filter projects list', 'delaware' ),
				'items_list_navigation' => esc_html__( 'Project list navigation', 'delaware' ),
				'items_list'            => esc_html__( 'Projects list', 'delaware' ),
			),
			'supports'            => array(
				'title',
				'editor',
		...
		register_taxonomy( $this->taxonomy_type, $this->post_type, array(
			'hierarchical'      => true,
			'labels'            => array(
				'name'                  => esc_html__( 'Portfolio Categories', 'delaware' ),
				...
				'search_items'          => esc_html__( 'Search Category', 'delaware' ),
				'items_list_navigation' => esc_html__( 'Category list navigation', 'delaware' ),
				'items_list'            => esc_html__( 'Category list', 'delaware' ),
			),
			'public'            => true,
			'show_ui'           => true,
			'show_in_nav_menus' => true,
	...
	public function edit_admin_columns( $columns ) {
		...
		$columns['title'] = esc_html__( 'Project', 'delaware' );
	...
	public function settings_api_init() {
		add_settings_section(
			'delaware_portfolio_section',
			'<span id="portfolio-options">' . esc_html__( 'Portfolio', 'delaware' ) . '</span>',
		...
		add_settings_field(
			$this->option,
			'<span class="portfolio-options">' . esc_html__( 'Portfolio', 'delaware' ) . '</span>',
		...
		if ( ! get_option( $this->option ) ) {
			...
			add_settings_field(
				...
				'<span class="portfolio-options">' . esc_html__( 'Portfolio page', 'delaware' ) . '</span>',
			...
			add_settings_field(
				...
				'<label for="portfolio_items_per_page">' . esc_html__( 'Portfolio items show at most', 'delaware' ) . '</label>',
			...
			add_settings_section(
				'delaware_portfolio_section',
				'<span id="portfolio-options">' . esc_html__( 'Portfolio Item Permalink', 'delaware' ) . '</span>',
			...
			add_settings_field(
				...
				'<label for="portfolio_category_slug">' . esc_html__( 'Portfolio category base', 'delaware' ) . '</label>',
	...
	public function permalink_section_html() {
		...
		$portfolio_page_id = get_option( $this->option . '_page_id' );
		$base_slug         = urldecode( ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug', 'delaware' ) );
		$portfolio_base    = _x( 'portfolio', 'Default slug', 'delaware' );
		...
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control the permalink used specifically for portfolio.', 'delaware' ); ?>
		</p>

		<table class="form-table delaware-portfolio-permalink-structure">
			<tbody>
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="delaware-portfolio-base" /> <?php esc_html_e( 'Default', 'delaware' ); ?>
					</label>
				</th>
				<td>
				...
				<tr>
					<th>
						<label><input name="portfolio_permalink" type="radio"
									  ...
									  class="delaware-portfolio-base" /> <?php esc_html_e( 'Portfolio base', 'delaware' ); ?>
						</label>
					</th>
					<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="delaware-portfolio-base" /> <?php esc_html_e( 'Portfolio base with category', 'delaware' ); ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" id="delaware_portfolio_custom_selection" type="radio"
								  value="custom" <?php checked( in_array( $portfolio_permalink, $structures ), false ); ?> /> <?php esc_html_e( 'Custom Base', 'delaware' ); ?>
					</label>
				</th>
				<td>
					<code><?php echo esc_html( home_url() ); ?></code>
					<input name="portfolio_permalink_structure" id="delaware_portfolio_permalink_structure" type="text"
		...
		<script type="text/javascript">
			jQuery( function () {
				jQuery( 'input.delaware-portfolio-base' ).change( function () {
					jQuery( '#delaware_portfolio_permalink_structure' ).val( jQuery( this ).val() );
				} );
				jQuery( '.permalink-structure input' ).change( function () {
					jQuery( '.delaware-portfolio-permalink-structure' ).find( 'code.non-default-example, code.default-example' ).hide();
					if ( jQuery( this ).val() ) {
						jQuery( '.delaware-portfolio-permalink-structure code.non-default-example' ).show();
						jQuery( '.delaware-portfolio-permalink-structure input' ).removeAttr( 'disabled' );
					} else {
						jQuery( '.delaware-portfolio-permalink-structure code.default-example' ).show();
						jQuery( '.delaware-portfolio-permalink-structure input:eq(0)' ).click();
						jQuery( '.delaware-portfolio-permalink-structure input' ).attr( 'disabled', 'disabled' );
					}
				} );
				jQuery( '.permalink-structure input:checked' ).change();
				jQuery( '#delaware_portfolio_permalink_structure' ).focus( function () {
					jQuery( '#delaware_portfolio_custom_selection' ).click();
				} );
			} );
		</script>
		<?php
	...
	public function disable_field_html() {
		...
		<label for="<?php echo esc_attr( $this->option ); ?>">
			...
			<?php esc_html_e( 'Disable Portfolio for this site.', 'delaware' ); ?>
		</label>
	...
	public function per_page_field_html() {
		...
		<label for="portfolio_posts_per_page">
			...
			<?php _ex( 'items', 'Portfolio items per page', 'delaware' ) ?>
		</label>
	...
	public function portfolio_category_slug_field_html() {
		...
		<input name="portfolio_category_slug" id="portfolio_category_slug" type="text"
			   ...
			   placeholder="<?php echo esc_attr( _x( 'portfolio-category', 'Portfolio category base', 'delaware' ) ) ?>"
		...
		if ( isset( $_POST['portfolio_permalink'] ) ) {
			...
			if ( 'custom' === $portfolio_permalink ) {
				...
				if ( '%portfolio_category%' == $portfolio_permalink ) {
					$portfolio_permalink = '/' . _x( 'portfolio', 'slug', 'delaware' ) . '/' . $portfolio_permalink;
				}
			...
			$portfolio_page_id   = get_option( $this->option . '_page_id' );
			$portfolio_permalink = ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug', 'delaware' );
	...
	public function portfolio_post_type_link( $permalink, $post ) {
		...
		if ( ! empty( $terms ) ) {
			...
			$portfolio_category = _x( 'uncategorized', 'slug', 'delaware' );
		}
